...
Expected: all eight fail — the route does not exist, so every request is a 404 (and the 404 test
...
Expected: daemon **36** (24 + 4 hub + 8 here), passing.
...
Expected: core 180, daemon **38**, cli 17, all passing; `make check` clean.
...
- [ ] `cargo test --workspace` passes: core 180, daemon 38, cli 17
...
- **`logs::search`.** Planned as H3 Task 2 and never implemented. Not H5's to fix, but it is the one
  piece of H3 still outstanding, and `known-gaps.md` still describes `search` as forthcoming.
