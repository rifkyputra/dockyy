...
- **Baselines to move deliberately, not by accident:** `kuadrat_core` **159**, `kuadrat_daemon`
  **24**, `kuadrat` (cli) **17**. `cargo test --workspace` prints one `test result:` line per crate.
...
Expected: core **163** (159 + 4 new), daemon 24, cli 17, all passing.
...
Expected: core **167** (163 + 3 from Task 2 + 1 here), passing.
...
Expected: core 167, daemon **38**, cli 17, all passing; `make check` clean.
...
- [ ] `cargo test --workspace` passes: core 167, daemon 38, cli 17
